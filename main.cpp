...
void set_score(int s)
{
    if (game_state != 0)
    {
        sampletick.attach(&play_success_sound, 1.0 / sample_freq);
        wait(1.0);
    }
    uLCD.cls();
...
void speak()
{
    ...
    while (timer.read() < timeout)
    {
        tmp = (mymicrophone - (0.67/3.3)) * 33 + 0.5;
        pc.printf("speak: %f\n", tmp);
        if (tmp > 0.70)
        {
            set_score(++score);
...
int main()
{
    Timer rand_t;
    rand_t.start();
    int high_score = LPC_RTC->GPREG0;
    if (high_score < 0 || high_score > 100) high_score = 0;
    int instruction;
